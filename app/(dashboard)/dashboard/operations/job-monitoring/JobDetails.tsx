...
const JobDetails = ({ jobId, onBack, onIntervene }: { jobId: string; onBack: () => void, onIntervene: () => void }) => {
   ...
   const isoString = jobData?.tdoDate;
    ...
    // Added type indicators for the pricing array
    const pricing = [
        {id:1, label: 'AGREED PRICE', value: jobData?.pricing?.agreedPrice, type: 'currency'},
        {id:2, label: 'PLATFORM COMMISION', value: jobData?.pricing?.platformFee, type: 'currency'},
        {id:3, label: 'TRUCKER PAYOUT ', value: jobData?.pricing?.truckerPayout, type: 'currency'},
        {id:4, label: 'MOBILISATION FEE (30%) ', value: jobData?.pricing?.mobilizationAmount, type: 'currency'},
        {id:5, label: 'MOBILISATION PAID ', value: jobData?.pricing?.mobilizationPaid, type: 'status'},
        {id:6, label: 'FINAL PAYOUT STATUS ', value: jobData?.pricing?.finalPayoutStatus, type: 'status'},
    ]
  ...
  return (
    <div className="p-5  mx-auto">
                 ...
                 <div className='mt-5 grid lg:grid-cols-3 gap-7'>
                    ...
                    <div className='grid gap-4 bg-white rounded-2xl py-6 px-3 col-span-2 '>
                        ...
                        <div >
                            ...
                            {pricing.map((p, index) => (
                                <div key={index}>
                                    {/* Pass the type prop down to DetailBox */}
                                    <DetailBox label={p.label} value={p.value} type={p.type as any} />
                                </div>
                            ))}
                            </div>
                        </div>
                    ...
                    <div className=' bg-white rounded-2xl py-6 px-3 '>
                        {/* role */}
                        <div>
                            {jobData?.forwarder && <UserRole name={jobData?.forwarder.name} role='Forwarder' email={jobData?.forwarder.email} id={jobData?.forwarder.id} onView={() => setSelectedUserId(jobData?.forwarder.id)} />}
                            {jobData?.trucker && <UserRole name={jobData?.trucker.name} role='Trucker' email={jobData?.trucker.email} id={jobData?.trucker.id} onView={() => setSelectedUserId(jobData?.trucker.id)} />}
                            {jobData?.driver && <UserRole name={jobData?.driver.name} role='Driver' email={jobData?.driver.email} id={jobData?.driver.id} onView={() => setSelectedUserId(jobData?.driver.id)} />}
                        </div>
                        ...
                        <div className='mt-7'>
                            ...
                            {[
                                ...
                                <div 
                                    key={index} 
                                    className='border border-[#718097] text-[#718097] rounded-lg text-xs font-bold px-3 py-1'
                                >
                                    {docConfig.label}
                                </div>
                                ))
                            ...
                            <div onClick={() => setDocModal(jobId)} className='border border-[#0241E8] rounded-lg text-[#0241E8] text-sm text-center py-2 mt-3 cursor-pointer'>
                                View
                            </div>
                        </div>
        ...
        <DocumentReviewModal jobId={docModal} onClose={() => setDocModal('')} onRefresh={() => {}} />
    </div>
  )
}
...
const DetailBox = ({ label, value, type = 'text' }: { label: string, value: any, type?: 'text' | 'currency' | 'status' }) => {
  
  // Format as Naira if the type is currency
  const displayValue = type === 'currency' && value !== undefined 
  ...
  return (
    <div className="bg-[#E8ECF1] h-full p-3.5 rounded-xl border border-slate-100">
      ...
      {/* Conditionally render the Status Pill or the Standard text/currency */}
      {type === 'status' ? (
        <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold ${
            value === 'Paid' 
            ? 'bg-[#E4FFF0] text-[#00652D]' 
            : 'bg-[#FFF6E5] text-[#EFA000]'
        }`}>
            <span className={`w-1.5 h-1.5 rounded-full ${value === 'Paid' ? 'bg-[#00652D]' : 'bg-[#EFA000]'}`}></span>
            {value || 'Pending'}
        </div>
      ) : (
...
const UserRole = ({ name,  email, role, id, onView }: { name: string, role: string, email: string, id: string, onView: () => void }) => (
  <div className="p-3.5">
    ...
    <div className='flex  items-center justify-between gap-2'>
        ...
        {/* Add the onClick event to trigger the view */}
        <div onClick={onView} className='border-[#0241E8] border rounded-lg px-3 py-1 cursor-pointer'>
            View
        </div>
